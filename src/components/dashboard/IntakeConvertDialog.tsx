...
export function IntakeConvertDialog({
  ...
  async function handleConvert(force = false) {
    ...
    try {
      ...
      toast.success("Submission converted to Partner + Brand + Contact.");
      onOpenChange(false);
      onSuccess();
    } catch (err) {
      const message =
        ...
        <div className="space-y-5 py-2">
          ...
          <section>
            ...
            <dl className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
              ...
              <dd>{submission.brandName}</dd>
              <dt className="text-muted-foreground">Brand Domain</dt>
              <dd>{submission.brandDomain || "—"}</dd>
              <dt className="text-muted-foreground">Tracking Domain</dt>
              <dd>{submission.trackingDomain || "—"}</dd>
              <dt className="text-muted-foreground">License Info</dt>
              <dd>{submission.licenseInfo || "—"}</dd>
              <dt className="text-muted-foreground">Target Geos</dt>
              <dd>
                {submission.targetGeos.length > 0 ? (
                  <div className="flex flex-wrap gap-1">
                    {submission.targetGeos.map((code) => (
          ...
          <div className="text-xs text-muted-foreground">
            Submitted {new Date(submission.submittedAt).toLocaleString()} — Generated by{" "}
